...
def get_parser():
    ...
    Create an HDF file containing spatially stacked ShakeMaps.  For example, if a given study area
    is contained in the box defined by longitude -100.129 to -95.592, and longitude 34.949 to 
    37.571, you can download all of the ShakeMaps in the 4 month period from 2017-01-01 to 2017-04-01, you
    would do this:

    %(prog)s oklahoma_2017.hdf -100.129 -95.592 34.949 37.571 2017-01-01 2017-04-01 -m 1.0 9.9 -r 0.01

    The resulting HDF5 file will contain stacked arrays, one per ground motion, at 0.1 degree resolution, 
    where "stacked" means that each ShakeMap found in the given bounds is resampled to a grid defined by
    those bounds and input resolution.  If there are 35 ShakeMaps found, then each stacked array will
    contain 35 layers, where each layer corresponds to an individual resampled ShakeMap.
    '''
    ...
    parser.add_argument('outputfile', metavar='OUTFILE', 
                        help='The name of the HDF file containing stacked ShakeMap grids.')
    parser.add_argument('lonmin', metavar='LONMIN',
                        type=float,
                        help='Western boundary of desired grid')
    parser.add_argument('lonmax', metavar='LONMAX',
                        type=float,
                        help='Eastern boundary of desired grid')
    parser.add_argument('latmin', metavar='LATMIN',
                        type=float,
                        help='Southern boundary of desired grid')
    parser.add_argument('latmax', metavar='LATMAX',
                        type=float,
                        help='Northern boundary of desired grid')
    parser.add_argument('start', type=maketime,
                        help='Start time for search (defaults to ~30 days ago). YYYY-mm-dd or YYYY-mm-ddTHH:MM:SS')
    parser.add_argument('end', type=maketime,
    ...
    parser.add_argument('-m','--mag-range', metavar=('minmag','maxmag'),dest='magRange', type=float,nargs=2,
                        help='Min/max magnitude to restrict search.',default=(5.5,9.9))
    parser.add_argument('-r','--resolution', metavar='RESOLUTION', type=float,default=0.01,
...
def main(args):
    ...
    for event in events:
        ...
        try:
            ...
            print('Error fetching ShakeMap grid from %s -  "%s".  Skipping.' % (event.id,str(e)))
        finally:
            os.remove(gridfile)
    ...
    for imtname,imtcube in imts.items():
        stack_file.setArray(imtname,imtcube,metadata=metadata,compression=True)
